   ...
   $('.button-background-h').remove();
      ...
      barContainer = myPlayer.controlBar;
        ...
        b1.on('click', function () {
            if(buttonBarState == 'on') {
                votaRating( videoData.idStreamSource, "http://new.tegme.tv/api/tegvalue", 10);//?
                disableButtonBar();
            }
        });
        ...
        b2.on('click', function () {
            if(buttonBarState == 'on') {
                votaRating(videoData.idStreamSource, "http://new.tegme.tv/api/tegvalue", 20);//?
                disableButtonBar();
            }
        });
        ...
        b3.on('click', function () {
            if(buttonBarState == 'on') {
                votaRating(videoData.idStreamSource, "http://new.tegme.tv/api/tegvalue", 30);//?
                disableButtonBar();
            }
        });
        ...
        b4.on('click', function () {
            if(buttonBarState == 'on') {
                votaRating(videoData.idStreamSource, "http://new.tegme.tv/api/tegvalue", 40);//?
                disableButtonBar();
            }
        });
        ...
        b5.on('click', function () {
            if(buttonBarState == 'on') {
                votaRating(videoData.idStreamSource, "http://new.tegme.tv/api/tegvalue", 50);//?
                disableButtonBar();
            }
        });
        ...
        b6.on('click', function () {//block
            if(buttonBarState == 'on') {
                updateView(videoData.idStreamSource, "http://new.tegme.tv/api/ViewUpdate", "blo");
                disableButtonBar();
            }
        });
        ...
        b7.on('click', function () {
            if(buttonBarState == 'on') {
                votaRating(videoData.idStreamSource, "http://new.tegme.tv/api/tegvalue", 60);//?
                disableButtonBar();
            }
        });

        b8.on('click', function () {
            var urlStar = "http://admin.tegme.tv/wizard/purchaseadv.aspx"+"?idsource="+videoData.idStreamSource;
            window.open(urlStar);
        });
    ...
    if( isIphone ){
      ...
      var iphone_buttons_interval = setInterval( function(){
        ...
        $('.vjs-button-star').css({'left' : '35px', 'bottom' : '0', 'background-color' : 'transparent!important', 'float': 'left', 'border-radius': '10px' }).attr("title","tooltip");
...
//alert(videoData);
        ...
        //aggiorna le visite
        updateView(currentVideo.idStreamSource,"http://new.tegme.tv/api/ViewUpdate", 'sou');
        ...
        if(currentVideo != null && currentVideo.hasadv) {
         //alert('hasadv');
            ...
            if(currentVideo.tipoadv == 'b') {
                ...
                //aggiorna le visite
                updateView(currentVideo.idadvsource,"http://new.tegme.tv/api/ViewUpdate",'adv');
...
//alert(videoData);
     ...
     * */
    Costanti_videoPath = "http://new.tegme.tv/public/channels/#/media/";
